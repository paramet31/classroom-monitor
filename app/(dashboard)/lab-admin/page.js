...
import { ROOMS, isRoomOccupied, parseRequestedSlot, autoAssignRoom } from '../../../lib/roomConfig';
...
export default function LabAdminPage() {
    ...
    const updateBookingStatus = async (id, newStatus) => {
        if (newStatus === 'approved') {
            ...
            if (!b.room) {
                alert('Cannot approve: No room assigned yet.');
                return;
            }
            const bSlot = parseRequestedSlot(b.requestedSlot);
            if (bSlot) {
                const isOccupied = isRoomOccupied(b.room, bSlot.day, bSlot.period, b.year, b.term, bookings.filter(x => x.id !== id));
                if (isOccupied) {
                    alert(`Cannot approve: ${b.room} is already booked on ${b.requestedSlot} by another approved request.`);
                    return;
                }
            }
        }
    ...
    const handleEditSave = async (e) => {
        ...
        if (editingBooking.status === 'approved' && editingBooking.room) {
            ...
            if (bSlot) {
                const isOccupied = isRoomOccupied(editingBooking.room, bSlot.day, bSlot.period, editingBooking.year, editingBooking.term, bookings.filter(x => x.id !== editingBooking.id));
                if (isOccupied) {
                    alert(`Cannot approve: ${editingBooking.room} is already booked on ${editingBooking.requestedSlot} by another approved request.`);
                    return;
                }
            }
        }
                    ...
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr><td colSpan="6" style={{textAlign: 'center', padding: '2rem', color: 'var(--text-muted)'}}>No requests found.</td></tr>
                                ...
                                <tr key={b.id}>
                                    <td>
                                        <div className={styles.flexRow}>
                                            ...
                                            <div>
                                                ...
                                                <div style={{fontSize: '0.75rem', color: 'var(--text-muted)'}}>{b.courseCode} ({b.program})</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <div style={{fontSize: '0.75rem', color: 'var(--text-muted)'}}>{b.section}</div>
                                    </td>
                                    <td>
                                        <select 
                                            value={b.status} 
